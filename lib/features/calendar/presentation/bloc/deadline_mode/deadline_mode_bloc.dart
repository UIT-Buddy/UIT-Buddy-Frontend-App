...
class DeadlineModeBloc extends Bloc<DeadlineModeEvent, DeadlineModeState> {
  DeadlineModeBloc({required GetDeadlineModeUsecase getDeadlineUsecase})
    : _getDeadlineUsecase = getDeadlineUsecase,
      super(const DeadlineModeState()) {
    on<DeadlineModeStarted>(_onDeadlineModeStarted);
    on<DeadlineModeEntitySelected>(_onDeadlineModeEntitySelected);
    on<DeadlineModeNextMonthSelected>(_onDeadlineModeNextMonthSelected);
    on<DeadlineModePreviousMonthSelected>(_onDeadlineModePreviousMonthSelected);
  }

  final GetDeadlineModeUsecase _getDeadlineUsecase;

  Future<void> _onDeadlineModeStarted(
    DeadlineModeStarted event,
    Emitter<DeadlineModeState> emit,
  ) async {
  ...
  void _onDeadlineModeEntitySelected(
    DeadlineModeEntitySelected event,
    Emitter<DeadlineModeState> emit,
  ) {
  ...
  Future<void> _onDeadlineModeNextMonthSelected(
    DeadlineModeNextMonthSelected event,
    Emitter<DeadlineModeState> emit,
  ) async {
    final currentEntity = state.calendarDeadlineEntity;
    if (currentEntity == null) return;
  ...
  Future<void> _onDeadlineModePreviousMonthSelected(
    DeadlineModePreviousMonthSelected event,
    Emitter<DeadlineModeState> emit,
  ) async {
    final currentEntity = state.calendarDeadlineEntity;
    if (currentEntity == null) return;
